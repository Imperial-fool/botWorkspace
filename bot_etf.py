...
class Bot:
    def __init__(self, coina, coinb, digit, tickSize, indicator, client, index, topbyvolume):
        ...
        if file_exists(self.bnb_api_pair+'.json') == True:
            with open(self.bnb_api_pair+'.json', 'r') as backprops:
    ...
    def save(self):
        ...
        with open(self.bnb_api_pair+'.json', 'w') as backprops:
    ...
    def calculateProfit(self):
     ...
     if size > 0 and size_buy > 0:
         ...
         self.profits.append(((float(self.limit_order_sell[-1]["price"]) - float(self.limit_order_buy[-1]["price"]))*float(self.limit_order_sell[-1]["executedQty"])) - self.fee*2)
    ...
    def binancebot(self):
     ...
     if upperbound + lowerbound > .5: #if bands are too close together it wont sell
         ...
         if ((self.factor < 0 and (self.buy_price[-1]*1.01 +(2*self.fee)) < self.result[2]['value']) or (self.factor < -2 )) or (self.buy_price[-1]*0.98 < self.result[2]['value'] and self.fib < 0 and self.factor < 0) and 10 >= self.ava_coinb*self.result[2]['value'] : #market sell at 1% gain
             self.sell(round_decimals_down(self.ava_coinb, self.decimal_places))
             return 0
             
         if (upperbound < 0.5 and (self.fib < 0 or self.factor <= -1) and (self.buy_price[-1]+(self.fee*2) <= self.result[2]['value']) and 10 >= self.ava_coinb*self.result[2]['value']): #sell when near upperband,factor is less than -1 and buy price is over trade fee + 0.025% profit
             self.sell(self.quantity())
             return 0
     if -2 <((self.buy_price[-1] - self.result[2]['value'])/((self.buy_price[-1]+self.result[2]['value'])/2))*100 and self.factor < 0:
             self.sell(round_decimals_down(self.ava_coinb,self.decimal_places))

     if  self.factor > 1 and self.ava_coina >= 15 and upperbound > 1 and self.fib > 0 : 
    ...
    def info(self):
        try:
            ...
            print(e)
            error_report(str(e),'binance api not responding')
    ...
    def mainloop(self):
         ...
         try:
            ...
            error_report(str(e),"issue with get_apidata()")
         try:
            self.binance_info()
         except Exception as e:
            print(e)
            print("issue with binance api")
            error_report(str(e),"issue with binance api")
         if self.count != 1:
          try:
           self.checkOrders()
          except ConnectionError as e:
            ...
            error_report(str(e),"connection error in checkorders")
         ...
         try:
          ...
          print(e)
          error_report(str(e),"error with getfactor()")
          
         try:
          self.binancebot()
         except Exception as e:
          print(e)
          error_report(str(e),"bot has issue with binancebot()")
...
def get_pairs():
    ...
    topbyvolume = sorted(topby, key=lambda x:x["symbol"])
    for i in topbyvolume:
        print(i['symbol'])

    time.sleep(30)
    spawnBots(topbyvolume)
...
#creates new bot class appends to bot list object
def spawnBots(topbyvolume):
 ...
 while True:
    for i in bots:
     try:
      ...
      counter = 0
     except Exception as e: #general catch all, bad pratice!
         print(e)
         error_report(str(e))
    profit_compute(seconds)
    dust_collector(bots)
...
def dust_collector(bots:list):
    if bots[0].count % 160 == 0: #this should be a function if it works!
        ...
        time.sleep(60)
def profit_compute(seconds:float):
    profit = 0
    for i in bots:
    ...
    if bots[0].count % 80 == 0:
        checkin(profit,seconds)

def checkin(profit:float, t:float):
    ...
    with smtplib.SMTP_SSL("smtp.gmail.com", port, context=cont) as server:
        ...
        accountValuebtc = float(info['snapshotVos'][0]['data']['totalAssetOfBtc'])
        avg_price = client.get_avg_price(symbol='BTCUSDT')
        accountValueUsd = accountValuebtc*float(avg_price["price"])
        message += ("Value of account in usd: " + str(accountValueUsd))
